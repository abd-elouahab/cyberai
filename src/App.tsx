...
const Navbar = ({ activePage, onNav }: { activePage: string; onNav: (p: string) => void }) => (
  <header className="fixed top-0 left-0 right-0 z-50 bg-ink h-16 flex items-center justify-between px-6 lg:px-12">
    ...
    <nav className="hidden md:flex items-center gap-8">
      {[
        ...
        <button 
          ...
          className={`text-[10px] font-bold uppercase tracking-[0.15em] transition-all pb-1 border-b-2 ${activePage === item.id ? 'text-gold border-gold' : 'text-sand/50 border-transparent hover:text-sand'}`}
        >
          {item.label}
        </button>
      ))}
      <button className="bg-clay hover:bg-rust text-white text-[10px] font-bold uppercase tracking-wider px-4 py-2 rounded-lg transition-all ml-4">
        Start Journey
      </button>
    </nav>
    <button className="md:hidden text-sand">
      <Menu className="w-6 h-6" />
    </button>
  </header>
);

const Hero = ({ title, sub, eyebrow, count, label }: { title: React.ReactNode; sub: string; eyebrow: string; count: number; label: string }) => (
  <section className="relative bg-ink overflow-hidden pt-32 pb-16 px-6 lg:px-12 mb-12">
    <div className="absolute inset-0 opacity-10 pointer-events-none" 
      style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23C17B4A' fill-opacity='0.4'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")` }} 
    />
    <motion.div 
    ...
    >
      ...
      <h1 className="font-serif text-5xl md:text-7xl font-bold text-sand leading-[1.1] mb-6">
        {title}
      </h1>
      <p className="text-sand/70 text-lg font-light leading-relaxed mb-8">
        {sub}
      </p>
      <div className="inline-flex items-center gap-2 bg-clay/20 border border-clay/30 px-4 py-1.5 rounded-full">
        <span className="text-clay text-xs font-bold leading-none">✦ {count} {label} available</span>
      </div>
    </motion.div>
  </section>
);

const ListingCard = ({ listing, onShowDetail, onDelete }: { listing: Listing; onShowDetail: (l: Listing) => void; onDelete: (id: number) => void }) => (
  <motion.div 
    layout
  ...
  >
    ...
    <div className="p-6">
      <h3 className="font-serif text-2xl font-bold text-ink mb-1">{listing.title}</h3>
...
const DetailView = ({ listing, onClose, onBook }: { listing: Listing; onClose: () => void; onBook: (b: any) => void }) => {
  ...
  const nights = Math.max(1, Math.ceil((new Date(formData.check_out).getTime() - new Date(formData.check_in).getTime()) / (1000 * 60 * 60 * 24)));
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row">
          <div className="flex-1 p-6 lg:p-12">
            <div className="rounded-[24px] overflow-hidden h-72 lg:h-[400px] mb-8 relative">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-ink/80 via-transparent to-transparent flex flex-col justify-end p-8">
                <h2 className="font-serif text-4xl lg:text-5xl font-bold text-sand mb-2">{listing.title}</h2>
                <p className="text-sand/80 flex items-center gap-1.5"><MapPin className="w-4 h-4" /> {listing.location}</p>
              </div>
            </div>
            ...
            <div className="space-y-12">
              <section>
                ...
                <p className="text-smoke leading-relaxed">
                  Immerse yourself in the living heritage of Morocco at <strong className="text-ink">{listing.title}</strong>. 
                  Nestled in {listing.location}, this is not just a place to sleep — it is an invitation to 
                  slow down, learn a craft, share a meal, and connect with the people who have 
                  preserved these traditions for generations.
                </p>
                <p className="text-smoke leading-relaxed mt-4">
                  Your host will guide you through an authentic <strong>{listing.activity}</strong> experience, 
                  sharing stories passed down through family and community.
                </p>
              </section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 bg-fog px-4 py-3 rounded-2xl text-sm text-smoke">
                      <Coffee className="w-4 h-4 text-clay" /> {item}
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="lg:w-[400px] bg-white lg:border-l border-clay/10 p-6 lg:p-10">
            {bookingDone ? (
              <motion.div 
              ...
              >
                ...
                <p className="text-green-800 text-sm mb-6">Booking Confirmed. We've sent the details to your email. Get ready for an unforgettable journey.</p>
                <div className="text-left space-y-2 text-sm text-green-900 bg-white/50 p-4 rounded-2xl">
                  <div className="flex justify-between"><span>Check-in</span> <span className="font-bold">{formData.check_in}</span></div>
                  <div className="flex justify-between"><span>Nights</span> <span className="font-bold">{nights}</span></div>
                  <div className="flex justify-between border-t border-green-200 pt-2 mt-2"><span>Total Paid</span> <span className="font-bold">${total}</span></div>
                </div>
                <button 
                  onClick={onClose}
              ...
              <div className="sticky top-10">
                <div className="mb-6">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-clay block mb-1">Price per night</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input 
                        ...
                        className="w-full bg-white border-2 border-clay/20 focus:border-clay rounded-xl px-3 py-2 text-sm outline-none transition-all" 
                      />
                      <input 
                        type="date" 
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-ink selection:bg-clay selection:text-white">
      <Navbar activePage={currentPage} onNav={setCurrentPage} />
      ...
      <main className="max-w-[1400px] mx-auto pb-32">
        {currentPage === "home" && (
          <>
            ...
            />
            <div className="px-6 lg:px-12 flex flex-col lg:flex-row gap-12">
                  ...
                  <div className="space-y-1">
                    {["All", ...regions].map(reg => (
                      <button 
                        ...
                        className={`block w-full text-left px-4 py-2 rounded-xl text-xs font-medium transition-all ${selectedRegion === reg ? 'bg-clay text-white shadow-md' : 'text-smoke hover:bg-fog'}`}
                      >
                        {reg}
                      </button>
                    ))}
                ...
                {listings.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {listings.map(listing => (
                      <ListingCard key={listing.id} listing={listing} onShowDetail={setSelectedListing} onDelete={deleteListing} />
                    ))}
                  </div>
                ) : (
                ...
                {stories.map(story => (
                  ...
                  >
                    <div className="flex items-center gap-4 mb-6">
                      <img src={story.avatar} alt={story.author} className="w-12 h-12 rounded-full border-2 border-clay/20" />
                      <div>
                    ...
                    <h3 className="font-serif text-2xl font-bold text-ink mb-3 group-hover:text-clay transition-colors">{story.title}</h3>
                  ...
                  >
                    ...
                    <form onSubmit={submitStory} className="space-y-4">
                      <input 
                        ...
                        required
                        className="w-full bg-fog/50 border-2 border-transparent focus:border-clay/30 rounded-2xl px-4 py-3 outline-none transition-all"
                      ...
                      <textarea 
                        ...
                        required
                        rows={4}
                        className="w-full bg-fog/50 border-2 border-transparent focus:border-clay/30 rounded-2xl px-4 py-3 outline-none transition-all resize-none"
                  ...
                  <div className="space-y-1">
                    {["All", ...activities].map(act => (
                      <button 
                        ...
                        className={`block w-full text-left px-4 py-2 rounded-xl text-xs font-medium transition-all ${selectedActivity === act ? 'bg-clay text-white shadow-md' : 'text-smoke hover:bg-fog'}`}
                      >
                        {act}
                      </button>
                    ))}
                  ...
                  {listings.map(exp => (
                    ...
                    >
                      ...
                      <div className="flex-1 p-8 flex flex-col justify-center">
                        ...
                        <h3 className="font-serif text-2xl font-bold text-ink mb-2">{exp.title}</h3>
                        <p className="text-smoke text-sm mb-6 flex items-center gap-1.5"><MapPin className="w-3 h-3" /> {exp.location}</p>
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-rust">${exp.price ?? 0} <span className="text-[10px] font-normal text-smoke">/pp</span></span>
                          <button 
      ...
      <footer className="bg-ink px-6 lg:px-12 py-20 text-center">
        ...
        <p className="text-sand/40 max-w-md mx-auto text-sm leading-relaxed mb-12">
          Preserving heritage through sustainable exploration.<br />Every journey supports a local artisan community.
        </p>
        ...
        <p className="text-[10px] text-sand/20 uppercase tracking-[0.2em]">
          © 2026 ATLAS CONNECT · ALL RIGHTS RESERVED
        </p>
      </footer>
